<?php
    class Job_model extends CI_Model{
        ...
        function get_job_requests() {
            $this->db->select('j.*,mv.name,concat(m.firstname ," ", m.lastname) as member_name,sp.company_name,js.status_txt');
            ...
            $this->db->where('j.sp_id',$this->session->userdata('id'));
        ...
        function getDataById($id){
            $this->db->select('j.*,mv.*,js.status_txt,sp.sp_id,sp.company_name,group_concat(DISTINCT(s.title)) as service_need_str,group_concat(DISTINCT(su.title)) as featured_services_upgrades_str');
        ...
        function chnange_status(){
            ...
            $data = array();
            $data['job_request_id'] = $this->input->post('id');
            $data['status'] = $status;
            if($this->db->insert('job_status_change',$data)){

                $update_st = array();
                $update_st['status'] = $status;
                $this->db->where('job_request_id',$this->input->post('id') );
                if($this->db->update('job_request',$update_st)){
                    $success = "Y";
                }
