...
class ZipcodeModel extends CI_Model {
    ...
    function getDataById($id){
        ...
        $categories = (object) [];
        if(!empty($row)){
            $query = $this->db->select('c.*')->from('zipcode_category as sc')->join('category as c','c.category_id=sc.category_id','left')->where('sc.zipcode_id',$row->id)->get();
    ...
    function create(){
        ...
        foreach($this->input->post('categories') as $val){
            ...
            $this->db->insert('zipcode_category',$data);
        }
    ...
    function update(){
        ...
        $this->db->where('zipcode_id', $this->input->post('id'));
        $this->db->delete('zipcode_category');

        foreach($this->input->post('categories') as $val){
            $data = array(
                'zipcode_id'=>$this->input->post('id'),
                'category_id'=>$val
            );
            $this->db->insert('zipcode_category',$data);
        }
